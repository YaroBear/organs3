...
var buildCollections = function() {

	console.log("Building ALL collections!");
	...
	mongodb.MongoClient.connect(mongodb_uri, function(error, db) {
		...
		donors.find().forEach (function(donor) {
			var organ = "";
			for (organ in donor.matching_info.organs[0]){};
			console.log(organ);
			hospitals.findOne({procedures : { $in :[organ] }}).then(function(result){
				if (result.doctors[0])
				{
				var doctor_id = result.doctors[0];
				doctors.findOneAndUpdate({_id : doctor_id}, {$push : {patients : donor._id}});
				}
			});
		});
	});
...
var destroyCollections = function() {
	...
	mongodb.MongoClient.connect(mongodb_uri, function(error, db) {
		...
		heart_waitlist.drop();
		lungs_waitlist.drop();
		kidney_waitlist.drop();
	});
}

exports.buildCollections = buildCollections;
exports.destroyCollections = destroyCollections;