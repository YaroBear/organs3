...
var Hospitals = schemas.Hospital;
...
router.get("/admin/api/collection/stats/:collectionName", (req, res) => {
    ...
    if (cName == 'recipients') {
        Recipients.collection.stats(function(err, results) {
            if (err) {
                res.send(err);
            } else {
                res.json(results);
    ...
    if (cName == 'hospitals') {
        Hospitals.collection.stats(function(err, results) {
            if (err) {
                res.send(err);
            } else {
                res.json(results);
    ...
    if (cName == 'doctors') {
        Doctors.collection.stats(function(err, results) {
            if (err) {
                res.send(err);
            } else {
                res.json(results);
...
router.post('/doctor/api/recipients', function(req, res) {
    ...
    Recipients.findOne({ ssn: request.ssn })

    .then(function(ssn) {
        if (ssn) {
            errors.ssnExists = "A Recipients with that SSN already exists";
        }
        ...
        res.status(201).send({ ok: true, message: 'Recipients added successfully' });
    }).catch(function(err) {
        console.log(err);
        res.status(500).send({ success: false, errors });
        res.status(500).send({ success: false, errors });
...
router.get("/doctor/api/recipentsByID/:id", (req, res) => {
    ...
    Recipients.findOne({ _id: ObjectId(id) }, (err, data) => {
        if (err) {
            res.send(err);
        } else {
            res.json(data);
...
router.post('/doctor/api/view-Recipients-donor-info', function(req, res) {
    var request = {};
    var response = {};
    // if req.body is empty (form is empty), use query parameters 
    // to test API without front end via Postman or regular xmlhttprequest
    ...
    Recipients.findOne({ "_id": ObjectId(request.recipient_id) })
        .then(function(Recipients) {
